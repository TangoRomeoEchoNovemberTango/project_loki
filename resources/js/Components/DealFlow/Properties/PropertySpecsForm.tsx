...
export const PropertySpecsForm: React.FC<PropertySpecsFormProps> = ({
  ...
  // Vacant land = no building, so the building specs grey out
  const specsDisabled = isLand;
  ...
  return (
    <div className="space-y-2 pt-1 border-t border-slate-800">
      ...
      <div className={`grid grid-cols-2 sm:grid-cols-5 gap-2 ${specsDisabled ? 'opacity-40' : ''}`}>
        <div>
          <span className="text-[10px] text-slate-400 block mb-0.5">Property Type</span>
          <select value={propertyType} disabled={specsDisabled} onChange={(e) => onTypeChange(e.target.value as PropertyType)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-white focus:border-amber-400 focus:outline-none disabled:cursor-not-allowed">
        ...
        <div>
          ...
          <input type="number" value={beds} disabled={specsDisabled} onChange={(e) => onBedsChange(e.target.value === '' ? '' : Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-white focus:border-amber-400 focus:outline-none disabled:cursor-not-allowed" />
        </div>
        <div>
          <span className="text-[10px] text-slate-400 block mb-0.5">Baths</span>
          <input type="number" step="0.5" value={baths} disabled={specsDisabled} onChange={(e) => onBathsChange(e.target.value === '' ? '' : Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-white focus:border-amber-400 focus:outline-none disabled:cursor-not-allowed" />
        </div>
        <div>
          <span className="text-[10px] text-slate-400 block mb-0.5">SqFt</span>
          <input type="number" value={sqft} disabled={specsDisabled} onChange={(e) => onSqftChange(e.target.value === '' ? '' : Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-white focus:border-amber-400 focus:outline-none disabled:cursor-not-allowed" />
        </div>
        <div>
          <span className="text-[10px] text-slate-400 block mb-0.5">Occupancy</span>
          <select value={occupancy} disabled={specsDisabled} onChange={(e) => onOccupancyChange(e.target.value as OccupancyStatus)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-white focus:border-amber-400 focus:outline-none disabled:cursor-not-allowed">

...
export const QuickAddPropertyForm: React.FC<QuickAddPropertyFormProps> = ({
  ...
  const handleSave = async () => {
    ...
    setIsCreating(true);
    try {
      const fullAddress = `${street.trim()}${unit.trim() ? ' ' + unit.trim() : ''}`;
      let createdLead: Lead | void | null = null;

      if (syncPipeline && onSaveLead) {
        createdLead = await onSaveLead({
          dealType, govListType: dealType === 'OFF_MARKET_GOV' ? govListType : undefined,
          propertyAddress: fullAddress, city: city || 'Springfield', state: state || 'IL', zip: zip || '62704',
          territoryId: territoryId || undefined,
          stage: pipelineStage || (dealType === 'OFF_MARKET_GOV' ? (mctpIsQualified ? 'MCTP_QUALIFIED' : 'NEW') : 'NEW'),
          contactName: `${contactFirstName} ${contactLastName}`.trim() || 'Unspecified Seller',
          contactFirstName: contactFirstName || '', contactLastName: contactLastName || '',
          contactPhone: contactPhone || currentContact.phone || '',
          contactRole: contactRole || currentContact.role || (dealType === 'OFF_MARKET_GOV' ? 'DIRECT_SELLER' : 'LISTING_AGENT'),
          valuation: { listPrice: Number(maoAskingPrice) || 0, estimatedArv: Number(maoArv) || 0, repairEstimate: Number(maoRepairs) || 0, calculatedMao, askingMaoGap: (Number(maoAskingPrice) || 0) - calculatedMao },
          mctp: { motivation: mctpMotivation || 'Phone Call Intake', condition: mctpCondition || 'Repairs needed', timeline: mctpTimeline || 'Immediate', askingPrice: Number(maoAskingPrice) || 0, sellerNetTarget: Number(mctpNetTarget) || 0, qualifiedDate: new Date().toISOString(), isQualified: mctpIsQualified },
          imageUrl: images[0], createdDate: new Date().toISOString().split('T')[0],
        });
      }

      if (onCreateProperty) {
        await onCreateProperty({
          streetAddress: street.trim(), unit: unit.trim() || undefined, city: city || 'Springfield', state: state || 'IL', zip: zip || '62704',
          territoryId: territoryId || undefined, propertyType, beds: Number(beds) || 0, baths: Number(baths) || 0, sqft: Number(sqft) || 0,
          occupancyStatus: occupancy, askingPrice: Number(maoAskingPrice) || 0, estimatedArv: Number(maoArv) || 0, estimatedRepairs: Number(maoRepairs) || 0,
          isLandDeal: isLand, acreage: Number(acreage) || undefined, hasMultipleStructures: hasMultiStructures,
          notes, // Added notes to the property payload
          leadId: createdLead ? (createdLead as Lead).id : undefined, images, attachments
        });
      }
  ...
  return (
    <div className="mt-3 p-4 bg-slate-900 border-2 border-amber-500/40 rounded-xl space-y-4 shadow-2xl">
      ...
      <DealTypeSelector dealType={dealType} govListType={govListType} onDealTypeChange={(type) => { setDealType(type); setContactRole(type === 'OFF_MARKET_GOV' ? 'DIRECT_SELLER' : 'LISTING_AGENT'); }} onGovListTypeChange={setGovListType} />
      ...
      {isLand && (
        <VacantLandForm
          land={landDetails}
          onChange={(u) => setLandDetails((prev) => ({ ...prev, ...u }))}
        />
      )}

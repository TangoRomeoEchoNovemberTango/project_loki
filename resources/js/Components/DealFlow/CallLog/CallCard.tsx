...
import type { CallLog } from '@/types/dealflow';
...
export const CallCard: React.FC<CallCardProps> = ({
  ...
  const [editOutcome, setEditOutcome] = useState(call.outcome || 'CONNECTED_INTERESTED');
    ...
    <div className="bg-slate-900/90 border border-slate-800 hover:border-slate-700/80 rounded-xl p-4 transition-all shadow-md space-y-3">
      ...
      {call.aiSummary && (
        <div className="bg-gradient-to-r from-amber-500/10 via-slate-900 to-amber-500/10 border border-amber-500/30 p-3.5 rounded-xl text-xs space-y-2">
          ...
          {call.aiSummary.propertyConditionNotes?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {call.aiSummary.propertyConditionNotes.map((note: any, idx: number) => (
                <span
                  key={idx}
